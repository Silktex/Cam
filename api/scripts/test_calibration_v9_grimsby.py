"""
Color Calibration Test Script v9 — Per-Image Adaptive Floor

Problem with v7: Fixed black subtraction crushes dark side images.
Problem with v8: Levels remap brightens everything — dark fabrics lose character.

Solution: Per-image adaptive floor.
  - Compute checker-derived max floor (same as v7)
  - For each image, cap the floor to MAX_FLOOR_FRACTION of that image's range
  - Bright images → full floor subtracted (proper black point)
  - Dark images → floor is capped proportionally (never crushes)
  - No artificial brightening — dark fabrics stay dark, light stay light

Pipeline per image:
  1. Load cropped TIFF (linear, 16-bit, fabric WB baked in)
  2. Apply WB shift + weighted diagonal color gains
  3. Per-image adaptive black floor (capped subtraction)
  4. Midtone crush (power factor for texture/grit)
  5. Clip to [0,1]
  6. Apply sRGB gamma
  7. Save as 16-bit PNG

Usage: python test_calibration_v9_grimsby.py [BATCH_NAME]
  Default: GRIMSBY-EARTH
  Example: python test_calibration_v9_grimsby.py GRIMSBY-MUSHROOM
"""
import sys
import logging
...
# ── Tunable parameters v9 (Adaptive) ─────────────────────────────────
NEUTRAL_WEIGHT = 10.0       # Same as v4/v7 (proven good color)
BLACK_OFFSET_SCALE = 0.62   # Same as v7 — max possible floor from checker
MIDTONE_CRUSH = 1.14        # Same as v7 — proven grit
MAX_FLOOR_FRACTION = 0.08   # NEW: floor can never exceed 8% of image's own range
                            # Bright top image (range ~0.5): floor capped at 0.04
                            # Dim side image (range ~0.15): floor capped at 0.012
                            # This prevents crushing dark images
...
def main():
    ...
    output_dir = batch_path / "color_calibrated_v9"
    debug_dir = output_dir / "_debug"
    v7_calib_dir = batch_path / "color_calibrated_v7"

    log.info("=" * 70)
    log.info(f"Color Calibration v9: {batch_name}")
    log.info("Adaptive: per-image floor capped to image range")
    log.info(f"  NEUTRAL_WEIGHT={NEUTRAL_WEIGHT}  BLACK_OFFSET={BLACK_OFFSET_SCALE}")
    log.info(f"  CRUSH={MIDTONE_CRUSH}  MAX_FLOOR_FRAC={MAX_FLOOR_FRACTION}")
    log.info("=" * 70)
    ...
    # ── Step 3: Checker-derived max floor ──
    log.info("\n--- Step 3: Checker Black Floor (max possible) ---")
    black_patch = detected[23]
    max_floor = black_patch * gains * BLACK_OFFSET_SCALE
    log.info(f"  Black patch (detected): R={black_patch[0]:.4f} G={black_patch[1]:.4f} B={black_patch[2]:.4f}")
    log.info(f"  Max floor (x{BLACK_OFFSET_SCALE}):     R={max_floor[0]:.4f} G={max_floor[1]:.4f} B={max_floor[2]:.4f}")
    log.info(f"  Per-image cap: {MAX_FLOOR_FRACTION*100:.0f}% of image P99 range")
    ...
    log.info(f"  Max floor:     R={max_floor[0]:.4f} G={max_floor[1]:.4f} B={max_floor[2]:.4f}")
    ...
    for tiff_path in sorted(cropped_dir.iterdir()):
        ...
        # 2. PER-IMAGE ADAPTIVE BLACK FLOOR
        #    Compute per-channel 99th percentile of this image
        img_p99 = np.percentile(calibrated, 99, axis=(0, 1))  # (3,)
        #    Cap floor to MAX_FLOOR_FRACTION of this image's range
        per_img_cap = img_p99 * MAX_FLOOR_FRACTION
        #    Use the smaller of checker floor and per-image cap
        adapted_floor = np.minimum(max_floor, per_img_cap)

        log.info(f"    P99:   R={img_p99[0]:.4f} G={img_p99[1]:.4f} B={img_p99[2]:.4f}")
        log.info(f"    Cap:   R={per_img_cap[0]:.4f} G={per_img_cap[1]:.4f} B={per_img_cap[2]:.4f}")
        log.info(f"    Floor: R={adapted_floor[0]:.4f} G={adapted_floor[1]:.4f} B={adapted_floor[2]:.4f}"
                 f"  {'(capped)' if np.any(per_img_cap < max_floor) else '(full)'}")

        # Subtract adapted floor
        calibrated = calibrated - adapted_floor.reshape(1, 1, 3)
        ...
        if count <= 1:
            ...
            cv2.imwrite(str(debug_dir / "_03_v9_calibrated.png"), out_bgr)
    ...
    if len(images_for_comp) >= 2:
        ...
        cv2.imwrite(str(debug_dir / "comparison_v7_v9.png"), comp)
        log.info(f"  Saved: comparison_v7_v9.png ({len(labels)} panels)")
    else:
