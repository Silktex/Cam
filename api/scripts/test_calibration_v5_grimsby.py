"""
Color Calibration Test Script for GRIMSBY-EARTH (v5)

Based on v4 with Gemini's refined tone mapping:
  1. Weighted least-squares: 10x weight on neutral patches (same as v4)
  2. Levels remap: instead of linear subtraction, remap black floor and
     re-stretch range — preserves lushness in shadows
  3. Sigmoid S-curve: replaces power-law crush — protects highlights
     while deepening shadows naturally
...
Pipeline per image:
  ...
  4. Levels remap: anchor black floor, re-stretch to [0,1]
  5. Sigmoid tone mapping (contrast + midpoint)
  6. Shadow neutralizer: warm shadows, desaturate deepest blacks
  7. Clip to [0,1]
  8. Apply sRGB gamma
  9. Save as 16-bit PNG

Input:  GRIMSBY-EARTH/cropped/*.tiff
Output: GRIMSBY-EARTH/color_calibrated_v5/ (16-bit PNGs, sRGB gamma)
"""
import sys
import logging
...
OUTPUT_DIR = BATCH_PATH / "color_calibrated_v5"
...
# ── Tunable parameters ──────────────────────────────────────────────────
NEUTRAL_WEIGHT = 15.0
BLACK_OFFSET_SCALE = 0.5       # Dropped from 0.9 — less heavy blacks
SIGMOID_CONTRAST = 1.1         # Softened from 1.4 — more visible texture
SIGMOID_MIDPOINT = 0.35        # Lowered from 0.50 — brighter midtones
SHADOW_THRESHOLD = 0.15        # Tightened from 0.20
SHADOW_WARMTH = np.array([1.04, 1.02, 0.90])
EXPOSURE_COMP = 1.2            # Global exposure lift before processing
...
def main():
    log.info("=" * 70)
    log.info("Color Calibration v5: GRIMSBY-EARTH")
    log.info("Levels remap + sigmoid + shadow neutralizer")
    ...
    for tiff_path in sorted(CROPPED_DIR.iterdir()):
        ...
        if count <= 1:
            ...
            # Final v5 calibrated
            cv2.imwrite(str(DEBUG_DIR / f"{tiff_path.stem}_03_v5_calibrated.png"), out_bgr)
    ...
    v5_top = next((f for f in OUTPUT_DIR.glob("*top*calibrated*")), None)

    if v5_top:
        new = cv2.imread(str(v5_top), cv2.IMREAD_UNCHANGED)
        for c, ch in enumerate(['B', 'G', 'R']):
            nm = new[:, :, c].astype(float).mean()
            log.info(f"  v5 {ch}: mean={nm:.0f}")
    ...
    log.info(f"  _03_v5_calibrated.png  — v5 final (levels + sigmoid)")
    log.info(f"  comparison_all_versions.png (v2/v3/v4/v5)")
    log.info(f"{'='*70}")
    return 0
