"""
Color Calibration Test Script for GRIMSBY-EARTH (v2)

Same approach as calibration_service.py:
  - White-point adaptation (scale reference to match detected illuminant)
  - Diagonal per-channel gains (least-squares)
  - WB shift to account for fabric vs checker WB difference

Pipeline per image:
  1. Load cropped TIFF (linear, 16-bit, fabric WB baked in)
  2. Apply WB shift (fabric WB -> checker WB)
  3. Apply diagonal color gains
  4. Clip to [0,1]
  5. Apply sRGB gamma
  6. Save as 16-bit PNG

Input:  GRIMSBY-EARTH/cropped/*.tiff
Output: GRIMSBY-EARTH/color_calibrated_v2/ (16-bit PNGs, sRGB gamma)
"""
import sys
import logging
...
OUTPUT_DIR = BATCH_PATH / "color_calibrated_v2"
...
def compute_diagonal_gains(detected, reference):
    """
    Standard calibration: white-point adaptation + diagonal gains.
    Same as calibration_service.py calibrate_image().
    ...
    # Diagonal gains: least-squares per channel
    gains = np.ones(3)
    for c in range(3):
        det_c = detected[:, c]
        ref_c = adapted_ref[:, c]
...
def main():
    log.info("=" * 70)
    log.info("Color Calibration v2: GRIMSBY-EARTH")
    log.info("White-point adaptation + diagonal gains (same as calibration_service)")
    log.info("=" * 70)
    ...
    # ── Step 2: Compute diagonal gains ──
    log.info("\n--- Step 2: Compute Diagonal Gains ---")
    gains, wp_scale, adapted_ref = compute_diagonal_gains(detected, reference)
    log.info(f"  White-point scale: R={wp_scale[0]:.4f} G={wp_scale[1]:.4f} B={wp_scale[2]:.4f}")
    log.info(f"  Diagonal gains:   R={gains[0]:.4f} G={gains[1]:.4f} B={gains[2]:.4f}")
    ...
    # ── Step 3: Compute WB shift ──
    log.info("\n--- Step 3: Fabric WB Shift ---")
    ...
    for tiff_path in sorted(CROPPED_DIR.iterdir()):
        ...
        if count <= 1:
            ...
            # Final calibrated
            cv2.imwrite(str(DEBUG_DIR / f"{tiff_path.stem}_02_calibrated.png"), out_bgr)
    ...
    if OLD_CALIB_DIR.exists():
        ...
        new_top = next((f for f in OUTPUT_DIR.glob("*top*calibrated*")), None)
        if old_top and new_top:
            old = cv2.imread(str(old_top), cv2.IMREAD_UNCHANGED)
            new = cv2.imread(str(new_top), cv2.IMREAD_UNCHANGED)
            if old.shape != new.shape:
                new = cv2.resize(new, (old.shape[1], old.shape[0]))

            for c, ch in enumerate(['B', 'G', 'R']):
                om = old[:, :, c].astype(float).mean()
                nm = new[:, :, c].astype(float).mean()
                log.info(f"  {ch}: old={om:.0f} new={nm:.0f} delta={nm-om:+.0f}")

            # Side-by-side mosaic
            sc = min(800 / old.shape[1], 800 / old.shape[0])
            sz = (int(old.shape[1] * sc), int(old.shape[0] * sc))
            o8 = (cv2.resize(old, sz) / 257).astype(np.uint8) if old.dtype == np.uint16 else cv2.resize(old, sz)
            n8 = (cv2.resize(new, sz) / 257).astype(np.uint8) if new.dtype == np.uint16 else cv2.resize(new, sz)
            d8 = np.clip(cv2.absdiff(o8, n8).astype(np.uint16) * 5, 0, 255).astype(np.uint8)
            comp = np.hstack([o8, n8, d8])
            cv2.putText(comp, "OLD (service)", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(comp, "NEW (v2 script)", (sz[0]+10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(comp, "DIFF x5", (sz[0]*2+10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.imwrite(str(DEBUG_DIR / "comparison_old_vs_new.png"), comp)
            log.info(f"  Saved: comparison_old_vs_new.png")
        else:
            log.info("  Missing top image in old or new folder")
    else:
        log.info("  No old calibration folder found")
