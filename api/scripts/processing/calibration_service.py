...
class CalibrationService:
    ...
    def save_profile(self, data: ColorCheckerData, name: str, overwrite: bool = False) -> str:
        ...
        # Extract checker WB at save time so it's always available
        # (checker_wb is a property of the lighting rig + sensor, constant across batches)
        checker_wb = []
        if checker_raw_path:
            try:
                from .raw_utils import extract_wb
                wb = extract_wb(checker_raw_path)
                if wb:
                    checker_wb = wb
                    logger.info(f"  Checker WB: {checker_wb}")
                else:
                    logger.warning("  Could not extract WB from checker RAW")
            except Exception as e:
