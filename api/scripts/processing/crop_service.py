...
class CropService:
    ...
    def get_top_image_for_crop(self, batch_path: str) -> Optional[Dict]:
        ...
        if webview_path.exists():
            ...
            # Thumbnail exists but no full_webview — generate one from source
            full_img = self._load_image(top_image, source_type)
            if full_img is None:
                return None
            orig_h, orig_w = full_img.shape[:2]

            # Generate full_webview (2400px) and cache it for future requests
            webview_dir = batch_path / "full_webview"
            webview_dir.mkdir(exist_ok=True)

            max_size = 2400
            preview_img = full_img.copy()

            # Convert 16-bit to 8-bit with gamma for display
            if preview_img.dtype == np.uint16:
                img_float = preview_img.astype(np.float32) / 65535.0
                img_float = np.power(np.clip(img_float, 0, 1), 1.0 / 2.2)
                preview_img = (img_float * 255).astype(np.uint8)

            if max(orig_h, orig_w) > max_size:
                s = max_size / max(orig_h, orig_w)
                preview_img = cv2.resize(preview_img, (int(orig_w * s), int(orig_h * s)), interpolation=cv2.INTER_AREA)

            preview_h, preview_w = preview_img.shape[:2]
            cv2.imwrite(str(webview_path), preview_img, [cv2.IMWRITE_JPEG_QUALITY, 92])
            logger.info(f"Generated full_webview for crop: {webview_path.name} ({preview_w}x{preview_h})")
    ...
    def _generate_thumbnail_base64(self, img: np.ndarray, max_size: int = 2400, apply_gamma: bool = True) -> str:
        """
