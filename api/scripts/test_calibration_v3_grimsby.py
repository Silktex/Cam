"""
Color Calibration Test Script for GRIMSBY-EARTH (v3)

Based on v2 (white-point adaptation + diagonal gains + WB shift)
with Gemini's suggested improvements:
  1. Weighted least-squares: 5x weight on neutral patches (18-23)
  2. Black point compensation: subtract black patch offset to anchor shadows
  3. Midtone crush: power factor before sRGB gamma to preserve depth
...
Output: GRIMSBY-EARTH/color_calibrated_v3/ (16-bit PNGs, sRGB gamma)
"""
import sys
import logging
...
OUTPUT_DIR = BATCH_PATH / "color_calibrated_v3"
...
# ── Tunable parameters ──────────────────────────────────────────────────
NEUTRAL_WEIGHT = 5.0       # Weight multiplier for neutral patches in least-squares
BLACK_OFFSET_SCALE = 0.5   # How much of the black patch to subtract (0=none, 1=full)
MIDTONE_CRUSH = 1.1        # Power factor before sRGB (>1 darkens midtones/shadows)
...
def main():
    log.info("=" * 70)
    log.info("Color Calibration v3: GRIMSBY-EARTH")
    log.info("Weighted gains + black point + midtone crush")
    ...
    for tiff_path in sorted(CROPPED_DIR.iterdir()):
        ...
        if count <= 1:
            ...
            # Final v3 calibrated
            cv2.imwrite(str(DEBUG_DIR / f"{tiff_path.stem}_03_v3_calibrated.png"), out_bgr)
    ...
    v3_top = next((f for f in OUTPUT_DIR.glob("*top*calibrated*")), None)

    if v3_top:
        new = cv2.imread(str(v3_top), cv2.IMREAD_UNCHANGED)
        for c, ch in enumerate(['B', 'G', 'R']):
            nm = new[:, :, c].astype(float).mean()
            log.info(f"  v3 {ch}: mean={nm:.0f}")

    # Build comparison if we have at least v2 and v3
    images_for_comp = []
    labels = []
    ...
    if v3_top:
        ...
        labels.append("v3 (weighted+BP+crush)")
    ...
    log.info(f"  _03_v3_calibrated.png  — v3 final (weighted+BP+crush)")
