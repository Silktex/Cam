"""
Color Calibration Test Script for GRIMSBY-EARTH (v6)

"Middle ground" between v4 (solid/heavy) and v5 (textured/bright).
Same pipeline as v5 but with tuned parameters for realistic fabric weight.

Pipeline per image:
  1. Load cropped TIFF (linear, 16-bit, fabric WB baked in)
  2. Apply WB shift + gains + exposure compensation
  3. Levels remap: anchor black floor, re-stretch to [0,1]
  4. Sigmoid tone mapping (contrast + midpoint)
  5. Shadow neutralizer: warm shadows, desaturate deepest blacks
  6. Clip to [0,1]
  7. Apply sRGB gamma
  8. Save as 16-bit PNG

Input:  GRIMSBY-EARTH/cropped/*.tiff
Output: GRIMSBY-EARTH/color_calibrated_v6/ (16-bit PNGs, sRGB gamma)
"""
import sys
import logging
...
OUTPUT_DIR = BATCH_PATH / "color_calibrated_v6"
...
V4_CALIB_DIR = BATCH_PATH / "color_calibrated_v4"
V5_CALIB_DIR = BATCH_PATH / "color_calibrated_v5"
...
# ── Tunable parameters ──────────────────────────────────────────────────
NEUTRAL_WEIGHT = 15.0
BLACK_OFFSET_SCALE = 0.65      # Middle ground (v4=0.7, v5=0.5) — anchors blacks
SIGMOID_CONTRAST = 1.25        # Middle ground (v4~1.2, v5=1.1) — gives back pop
SIGMOID_MIDPOINT = 0.32        # Slightly darker than v5's 0.35
SHADOW_THRESHOLD = 0.15
SHADOW_WARMTH = np.array([1.04, 1.02, 0.90])
EXPOSURE_COMP = 1.1            # Dropped from v5's 1.2 — more realistic brightness
...
def apply_sigmoid(x, contrast=1.2, midpoint=0.45):
    """
    Sigmoid S-curve tone mapping.
    Adds 'bite' to midtones while protecting highlights and shadows.
    """
    s = 1.0 / (1.0 + np.exp(-contrast * 10.0 * (x - midpoint)))
    # Normalize so that 0 maps to 0 and 1 maps to 1
    s_at_0 = 1.0 / (1.0 + np.exp(-contrast * 10.0 * (0.0 - midpoint)))
    s_at_1 = 1.0 / (1.0 + np.exp(-contrast * 10.0 * (1.0 - midpoint)))
    return (s - s_at_0) / (s_at_1 - s_at_0)
...
def main():
    log.info("=" * 70)
    log.info("Color Calibration v6: GRIMSBY-EARTH")
    log.info("Middle ground: v4 weight + v5 texture")
    log.info(f"  NEUTRAL_WEIGHT={NEUTRAL_WEIGHT}  BLACK_OFFSET={BLACK_OFFSET_SCALE}")
    log.info(f"  SIGMOID: contrast={SIGMOID_CONTRAST}  midpoint={SIGMOID_MIDPOINT}")
    log.info(f"  SHADOW: threshold={SHADOW_THRESHOLD}  warmth=R{SHADOW_WARMTH[0]}/G{SHADOW_WARMTH[1]}/B{SHADOW_WARMTH[2]}")
    log.info(f"  EXPOSURE_COMP={EXPOSURE_COMP}")
    log.info("=" * 70)
    ...
    # ── Step 3: Compute black floor for levels remap ──
    log.info("\n--- Step 3: Black Floor (Levels Remap) ---")
    black_patch = detected[23]
    black_floor = black_patch * gains * BLACK_OFFSET_SCALE
    ...
    log.info(f"  Floor (x{BLACK_OFFSET_SCALE}):          R={black_floor[0]:.4f} G={black_floor[1]:.4f} B={black_floor[2]:.4f}")
    log.info(f"  Remap formula: out = (in - floor) / (1 - floor)")
    ...
    log.info(f"  Black floor:   R={black_floor[0]:.4f} G={black_floor[1]:.4f} B={black_floor[2]:.4f}")
    log.info(f"  Sigmoid:       contrast={SIGMOID_CONTRAST} midpoint={SIGMOID_MIDPOINT}")

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    DEBUG_DIR.mkdir(parents=True, exist_ok=True)

    b_floor = black_floor.reshape(1, 1, 3)
    ceiling = 1.0 - b_floor  # per-channel denominator for levels remap
    ...
    for tiff_path in sorted(CROPPED_DIR.iterdir()):
        ...
        # 1. Apply WB shift + gains + exposure lift
        calibrated = (img_f * combined.reshape(1, 1, 3)) * EXPOSURE_COMP

        # 2. Levels remap: anchor black floor and re-stretch to [0,1]
        calibrated = (calibrated - b_floor) / ceiling
        calibrated = np.clip(calibrated, 0, 1)

        # 3. Sigmoid tone mapping
        calibrated = apply_sigmoid(calibrated, SIGMOID_CONTRAST, SIGMOID_MIDPOINT)
        calibrated = np.clip(calibrated, 0, 1)

        # 4. Shadow neutralizer — kill blue cast in deep shadows
        # Luminance (Rec.709 weights)
        luma = 0.2126 * calibrated[..., 0] + 0.7152 * calibrated[..., 1] + 0.0722 * calibrated[..., 2]
        luma = np.expand_dims(luma, axis=-1)

        # Shadow mask: smooth ramp targeting bottom SHADOW_THRESHOLD of luminance
        shadow_mask = np.clip(1.0 - (luma / SHADOW_THRESHOLD), 0, 1)

        # Warm the shadows: reduce blue, boost red/green slightly
        calibrated = calibrated * (1.0 + shadow_mask * (SHADOW_WARMTH.reshape(1, 1, 3) - 1.0))

        # Force desaturation in the deepest blacks (kill purple/blue noise)
        calibrated = calibrated * (1.0 - shadow_mask) + luma * shadow_mask
        calibrated = np.clip(calibrated, 0, 1)
        ...
        if count <= 1:
            ...
            # After gains + levels remap only (no sigmoid)
            cal_no_sig = img_f * combined.reshape(1, 1, 3)
            cal_no_sig = np.clip((cal_no_sig - b_floor) / ceiling, 0, 1)
            cal_no_sig_srgb = linear_to_srgb(cal_no_sig)
            cv2.imwrite(str(DEBUG_DIR / f"{tiff_path.stem}_02_levels_only.png"),
                       cv2.cvtColor((cal_no_sig_srgb * 65535).astype(np.uint16), cv2.COLOR_RGB2BGR))

            # Final v6 calibrated
            cv2.imwrite(str(DEBUG_DIR / f"{tiff_path.stem}_03_v6_calibrated.png"), out_bgr)
    ...
    if V4_CALIB_DIR.exists():
        v4_top = next((f for f in V4_CALIB_DIR.glob("*top*calibrated*")), None)
    if V5_CALIB_DIR.exists():
        v5_top = next((f for f in V5_CALIB_DIR.glob("*top*calibrated*")), None)
    v6_top = next((f for f in OUTPUT_DIR.glob("*top*calibrated*")), None)

    if v6_top:
        new = cv2.imread(str(v6_top), cv2.IMREAD_UNCHANGED)
        for c, ch in enumerate(['B', 'G', 'R']):
            nm = new[:, :, c].astype(float).mean()
            log.info(f"  v6 {ch}: mean={nm:.0f}")

    # Build comparison: v4 vs v5 vs v6
    images_for_comp = []
    labels = []

    if v4_top:
        img = cv2.imread(str(v4_top), cv2.IMREAD_UNCHANGED)
        if img is not None:
            images_for_comp.append(img)
            labels.append("v4")

    if v5_top:
        img = cv2.imread(str(v5_top), cv2.IMREAD_UNCHANGED)
        if img is not None:
            images_for_comp.append(img)
            labels.append("v5")
    ...
    if len(images_for_comp) >= 2:
        ...
        x = 10
        for label in labels:
            cv2.putText(comp, label, (x, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            x += sz[0]
    ...
    log.info(f"  _02_levels_only.png    — gains + levels remap (no sigmoid)")
    log.info(f"  _03_v6_calibrated.png  — v6 final (middle ground)")
    log.info(f"  comparison_all_versions.png (v4/v5/v6)")
    log.info(f"{'='*70}")
    return 0
