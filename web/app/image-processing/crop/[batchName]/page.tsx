...
import {
  ...
  RectangleHorizontal, Square, ChevronDown, ChevronRight, Waves
} from 'lucide-react';
import {
...
export default function ImageProcessingCropPage() {
  ...
  const drawCanvas = useCallback(() => {
    ...
    // Set canvas backing store to full device resolution
  ...
  // Make angles 90deg (rectangularize) - keeps center and average dimensions
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        // After crop succeeds, apply straighten if configured
        ...
        // Save crop data to process_track.json
        ...
        setTimeout(() => router.push('/image-processing'), 1500);
      } else {
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/image-processing" className="text-teal-400 hover:text-teal-300">
    ...
    <div className="min-h-screen bg-slate-900 flex">
      ...
      <div className="w-80 bg-slate-800 border-l border-slate-700 flex flex-col shrink-0">
        ...
        <div className="p-4 border-b border-slate-700">
          ...
          <div className="text-xl font-semibold text-white">{dims.width} x {dims.height} <span className="text-sm font-normal text-slate-500">px</span></div>
          <div className="text-xs text-slate-500 mt-1">Original: {imageData.width} x {imageData.height}</div>
        </div>
        ...
        <div className="p-4 border-b border-slate-700">
          <button
            onClick={() => setStraightenExpanded(!straightenExpanded)}
          ...
          >
            ...
            {straightenExpanded ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
          </button>
          ...
          {straightenExpanded && (
            <div className="mt-3 space-y-3">
              {/* Analyze button */}
              <button
                onClick={handleStraightenAnalyze}
                disabled={straightenProcessing !== null}
                className="w-full flex items-center justify-center gap-2 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-500 disabled:opacity-50 text-sm"
              >
                {straightenProcessing === 'analyze' ? <Loader2 className="w-4 h-4 animate-spin" /> : <Wand2 className="w-4 h-4" />}
                Analyze Yarns
              </button>
              ...
              {straightenAnalysis && (
                <div className="bg-slate-700/50 rounded-lg p-3 text-xs space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="text-white">{straightenAnalysis.skew_angle_deg?.toFixed(2)}deg</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Weft bow:</span>
                    <span className="text-white">{straightenAnalysis.max_weft_bow_px?.toFixed(1)}px</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">Warp bow:</span>
                    <span className="text-white">{straightenAnalysis.max_warp_bow_px?.toFixed(1)}px</span>
                  </div>
                  {straightenAnalysis.recommendation && (
                    <div className="text-slate-400 pt-1 border-t border-slate-600">{straightenAnalysis.recommendation}</div>
                  )}
                </div>
              )}
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-1">
                  {['auto', 'skew', 'bow'].map(m => (
                    <button
                      key={m}
                      onClick={() => setStraightenMode(m)}
                      className={`py-1.5 rounded text-xs capitalize ${straightenMode === m ? 'bg-indigo-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
                    >
                      {m}
                    </button>
                  ))}
              ...
              <div>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="text-white">{Math.round(straightenStrength * 100)}%</span>
                </div>
                <input
                  type="range" min="0" max="1" step="0.05"
                  value={straightenStrength}
                  onChange={e => setStraightenStrength(parseFloat(e.target.value))}
                  className="w-full h-2 rounded-lg cursor-pointer"
                  style={{ background: `linear-gradient(to right, #6366f1 0%, #6366f1 ${straightenStrength * 100}%, #334155 ${straightenStrength * 100}%, #334155 100%)` }}
                />
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-1">
                  {['both', 'warp', 'weft'].map(d => (
                    <button
                      key={d}
                      onClick={() => setStraightenDirection(d)}
                      className={`py-1.5 rounded text-xs capitalize ${straightenDirection === d ? 'bg-indigo-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
                    >
                      {d}
                    </button>
                  ))}
              ...
              <div>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="text-white">{straightenGrid}</span>
                </div>
                <input
                  type="range" min="10" max="40" step="1"
                  value={straightenGrid}
                  onChange={e => setStraightenGrid(parseInt(e.target.value))}
                  className="w-full h-2 rounded-lg cursor-pointer"
                  style={{ background: `linear-gradient(to right, #6366f1 0%, #6366f1 ${((straightenGrid - 10) / 30) * 100}%, #334155 ${((straightenGrid - 10) / 30) * 100}%, #334155 100%)` }}
                />
              </div>

              {/* Manual angle override */}
              <div>
                <label className="text-xs text-slate-400 block mb-1">Manual Angle Override</label>
                <div className="flex items-center gap-2">
                  <input
                    type="number" step="0.1" min="-45" max="45"
                    value={manualSkewAngle ?? ''}
                    placeholder="Auto"
                    onChange={e => setManualSkewAngle(e.target.value ? parseFloat(e.target.value) : null)}
                    className="flex-1 px-2 py-1.5 text-xs bg-slate-700 border border-slate-600 rounded-lg text-white"
                  />
                  <span className="text-xs text-slate-500">deg</span>
                </div>
              </div>

              {/* Preview / Undo buttons */}
              <div className="flex gap-2">
                <button
                  onClick={handleStraightenPreview}
                  disabled={straightenProcessing !== null}
                  className="flex-1 flex items-center justify-center gap-1 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-500 disabled:opacity-50 text-sm"
                >
                  {straightenProcessing === 'preview' ? <Loader2 className="w-3 h-3 animate-spin" /> : null}
                  Preview
                </button>
                <button
                  onClick={handleStraightenUndo}
              ...
              {/* Preview images */}
              {straightenPreviewData && straightenPreviewData.before_url && (
                <div className="space-y-2">
                  <div className="text-xs text-slate-400">Before / After</div>
                  <div className="grid grid-cols-2 gap-2">
                    <img src={getFullUrl(straightenPreviewData.before_url)} alt="Before" className="rounded border border-slate-600" />
                    <img src={getFullUrl(straightenPreviewData.after_url)} alt="After" className="rounded border border-slate-600" />
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="p-4 border-t border-slate-700 space-y-2">
          ...
          <button
            onClick={() => router.push('/image-processing')}
